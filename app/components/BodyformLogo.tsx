...
interface LogoProps {
  size?: "sm" | "md" | "lg" | "xl";
  animated?: boolean;
  /** Chemin vers le fichier logo dans /public (ex: "/logo.gif", "/logo.mp4", "/logo.png") */
  src?: string;
}
...
function LogoMedia({ src, size }: { src: string; size: number }) {
...
export default function BodyformLogo({ size = "md", animated = false, src }: LogoProps) {
  const s = sizes[size];
  return (
    <div
    ...
    >
      {src ? (
        <LogoMedia src={src} size={s.figure} />
      ) : (
        <>
          {/* Silhouette musculaire SVG de fallback */}
          <svg
            width={s.figure}
            height={s.figure}
            viewBox="0 0 200 200"
